...
if ($inSAMFile eq "-")
{
    open($fin, "<&STDIN") or die "ERROR: Failed to read from STDIN\n";
    
    # Force reading from stdin correctly
    my @lines = <$fin>;
    close($fin);

    # Reopen a temporary file handle to simulate a normal file input
    open($fin, "<", \join("", @lines)) or die "ERROR: Failed to reopen input stream\n";
}
else
{
	if ($inSAMFile =~/\.gz$/)
...
while (my $line = <$fin>)
{
	...
	my $uniq = 0;
	$uniq = 1 if ($sam->{"TAGS"} =~ /XT:A:U/ || $sam->{"MAPQ"} == 255);
